...
def fda_device_registration_listings_page(request):
    page_data = get_sorted_data(request, 'fda_device_registrationlisting', 'registration.registration_number', 500,
                                ['registration.registration_number', 'registration.fei_number', 'registration.name',
                                 'upload_at'])
...
def fda_food_enforcement_page(request):
    page_data = get_sorted_data(request, 'fda_food_enforcement', 'recall_number', 500,
                                ['recall_number', 'event_id', 'recalling_firm', 'upload_at', 'termination_date',
...
def fda_food_event_page(request):
    page_data = get_sorted_data(request, 'fda_food_event', 'report_number', 500,
                                ['report_number', 'upload_at', 'products'])
    # page_data['context_data'][] = page_data.get('conte')
    print(page_data.get('context_data')[:3])
    for i in page_data.get('context_data'):
        print(i)
        i['products'] = mark_safe(json2html.json2html.convert(i.get('products'),table_attributes="class = \"table table-hover table table-bordered\""))
